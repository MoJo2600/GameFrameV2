...
void irReceiver()
{
  ...
  if (irrecv.decode(&results))
  {
    ...
    else if (results.value == 4294967295 && millis() - prevRemoteMillis < 250) // require recent prime signal to assume repeat
    {
      ...
      if (irLastCommand == 'M')
        irMenuRepeat = true;
      else if (irLastCommand == 'N')
        irNextRepeat = true;
      else if (irLastCommand == 'P')
        irPowerRepeat = true;
    }
    else
      understood = false;
...
void applyCurrentABC()
{
  ...
  for (byte x = 0; x < 9; x++)
  {
    if (dayInMinutes > triggerTimes[x] && triggerTimes[x] != -1)
      newBrightness = BrightnessSettings[x];
  }
  if (newBrightness == -1)
  {
    ...
    for (byte x = 1; x < 9; x++)
    {
      if (triggerTimes[x] > triggerTimes[x - 1])
        newBrightness = BrightnessSettings[x];
    }
  }
  if (newBrightness > -1)
...
void initClock()
{
  ...
  {
    // 24 hour conversion
    if (hour12 && currentHour > 12)
      currentHour -= 12;
    if (hour12 && currentHour == 0)
      currentHour = 12;
    drawDigitsAndShow();
  }
}
...
void powerControl()
{
  ...
  else if (digitalRead(buttonMenuPin) == HIGH && buttonPressed == true)
    buttonPressed = false;
  // power on
...
void breakoutLoop()
{
  ...
  else if ((digitalRead(buttonNextPin) == HIGH || digitalRead(buttonMenuPin) == HIGH || irCommand == 'M' || irCommand == 'N') && gameInitialized == false)
  ...
  if (ballMoving == true && fileIndex == 0)
  {
    ...
    else
    {
      if (ballAngle < 180)
      {
        if ((ballX + sin(degToRad(ballAngle)) * 16) + .5 > 256)
          swapXdirection();
      }
      else
      {
        if ((ballX + sin(degToRad(ballAngle)) * 16) + .5 < 0)
          swapXdirection();
      }
      ...
      {
        if ((ballY + cos(degToRad(ballAngle)) * 16) + .5 > 256)
          swapYdirection();
      }
